...
const IlkModal = ({ onIlkModalSubmit, onClose, ilkModalBilgi, isPeriyodikBakimMode = false }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center backdrop-blur-sm">
      <div className="dark-card-bg neumorphic-card rounded-3xl max-w-2xl w-full mx-4 md:mx-0 max-h-[90vh] overflow-y-auto p-4">
        <div className="flex justify-between items-center p-5 border-b dark-border rounded-t-lg">
          <h3 className="text-xl font-medium dark-text-primary">
            {isPeriyodikBakimMode ? 'Periyodik Bakım Kartı Ekle' : 'Kart Ekle'}
          </h3>
          <button onClick={onClose} className="dark-text-muted hover:dark-text-primary">
        ...
        <div className="p-4 md:p-8 md:pl-16 md:pr-16">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" id="adSoyad" placeholder="Ad Soyad" value={adSoyad} onChange={e => setAdSoyad(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md" />
            <input type="number" id="telNo" placeholder="Telefon No" pattern="\d{10}" value={telNo} onChange={e => setTelNo(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md" />
            ...
            <select 
              ...
              className="neumorphic-input p-2 dark-text-primary font-medium rounded-md"
            >
            ...
            <input type="date" id="girisTarihi" placeholder="Giriş Tarihi" value={girisTarihi} onChange={e => setGirisTarihi(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md" />
            <input type="text" id="plaka" placeholder="Plaka" value={plaka} onChange={e => setPlaka(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md" />
            <input type="number" id="km" placeholder="Km" value={km} onChange={e => setKm(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md" />
            <input type="text" id="sasi" placeholder="Şasi No" value={sasi} onChange={e => setSasi(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md md:col-span-2" />
            ...
            <textarea placeholder="Adres" id="adres" value={adres} onChange={e => setAdres(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md md:col-span-2" rows="3"></textarea>
            <textarea placeholder="Notlar" id="notlar" value={notlar} onChange={e => setNot(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md md:col-span-2" rows="3"></textarea>
            <div className="flex items-center gap-6 md:col-span-2">
              ...
              <div className="flex items-center gap-2">
                <input 
                  ...
                  className={`w-4 h-4 text-blue-400 dark-bg-tertiary dark-border rounded focus:ring-blue-400 ${isPeriyodikBakimMode ? 'opacity-50 cursor-not-allowed' : ''}`}
                />
                <label htmlFor="periyodikBakim" className={`dark-text-primary font-medium ${isPeriyodikBakimMode ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                  Periyodik Bakım {isPeriyodikBakimMode && '(Zorunlu)'}
                </label>
              </div>
            </div>
            <input type="text" id="duzenleyen" placeholder="Düzenleyen *" value={duzenleyen} onChange={e => setDuzenleyen(e.target.value)} className="neumorphic-input p-2 dark-text-primary font-medium rounded-md md:col-span-2" required />
          </div>
